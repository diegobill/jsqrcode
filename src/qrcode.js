...
jsqrcode.decode = function(src){
    ...
    else
    {
        ...
        image.onload=function(){
            ...
            if(image.width*image.height>jsqrcode.maxImgSize)
            {
                var ir = image.width / image.height;
                nheight = Math.sqrt(jsqrcode.maxImgSize/ir);
                nwidth=ir*nheight;
            }
            ...
            jsqrcode.height = canvas_qr.height;
            try{
                jsqrcode.imagedata = context.getImageData(0, 0, canvas_qr.width, canvas_qr.height);
            }catch(e){
...
jsqrcode.isUrl = function(s)
{
    var regexp = /(ftp|http|https):\/\/(\w+:{0,1}\w*@)?(\S+)(:[0-9]+)?(\/|\/([\w#!:.?+=&%@!\-\/]))?/;
    return regexp.test(s);
}

jsqrcode.decode_url = function (s)
{
  var escaped = "";
  try{
    escaped = escape( s );
